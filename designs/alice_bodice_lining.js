...
const steps = [
    ...
    {
      description: (status => {return `Point K is the Blade (or "front of arm") ${printMeasure(status.measurements.blade)} left of A1. Square line up and down to I and J`}),
      action : (status) => {
        let pointA1 = status.pattern.points['A1'];
        let pointO = status.pattern.points['O'];
        let pointB = status.pattern.points['B'];

        let pointK = setPoint(pointA1.x - inchesToPrecision(status, parseFloat(status.measurements.blade.value)), pointA1.y, { d: true, u: true });
        let pointI = setPoint(pointK.x, pointO.y);
        let pointJ = setPoint(pointK.x, pointB.y);

        status = registerPoints(status, {'K': pointK, 'I': pointI, 'J': pointJ});
    ...
    {
      description: (_status) => { return `Point D is 1 1/8 inch left from C. Draw a guide line from C to X. 11 is where this guide line crosses the line from A1` },
      action: (status) => {
        let pointC = status.pattern.points['C'];
        let pointX = status.pattern.points['X'];
        let pointA = status.pattern.points['A'];

        status.pattern.points['D'] = setPoint(pointC.x - inchesToPrecision(status, 1.125), pointC.y);
        ...
        status = status = setCurve(status, {s: '8', g: '11', e: 'D'});
        return status;
      }
    },
    {
